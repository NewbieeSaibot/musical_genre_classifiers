...
DATASET_PATH = "./genres" #input
JSON_PATH = "data.json" #output
SAMPLE_RATE = 22050
DURATION = 30
SAMPLES_PER_TRACK = SAMPLE_RATE * DURATION

def save_mfcc(dataset_path, json_path, n_mfcc=13, n_fft= 2048, hop_length=512, num_segments = 5):
    #build a dictionaty to store the data
    data = {
        "mapping": [],
        "mfcc": [],
        "labels": [],
    ...
    for i, (dirpath, dirname, filenames) in enumerate(os.walk(dataset_path)):
        ...
        if dirpath is not dataset_path:
            ...
            semantic_label = dirpath_components[-1] # a cool trick to take the alst element of the array
            data["mapping"].append(semantic_label)

            print("\nprocessing {}".format(semantic_label))
            #process files for a specif genre
            for f in filenames:
